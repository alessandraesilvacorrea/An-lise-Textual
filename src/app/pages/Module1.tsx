...
type Topic = 'narrativo' | 'descritivo' | 'dissertativo' | 'injuntivo' | 'expositivo' | null;
...
export default function Module1() {
  ...
  const topics = [
    { id: 'narrativo', name: 'Narrativo', icon: '📖', cards: narrativoCards },
    { id: 'descritivo', name: 'Descritivo', icon: '🎨', cards: descritivoCards },
    { id: 'dissertativo', name: 'Dissertativo', icon: '💭', cards: dissertativoCards },
    { id: 'injuntivo', name: 'Injuntivo', icon: '📋', cards: injuntivoCards },
    { id: 'expositivo', name: 'Expositivo', icon: '📚', cards: expositivoCards },
  ];
  ...
  const handleTopicComplete = async (topicId: string) => {
    ...
    try {
      ...
      if (next.size === topics.length) {
        alert('🎉 Parabéns! Você completou o Módulo 1!');
        navigate('/home');
      }
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Escolha um tipo textual para estudar</h2>
          <p className="text-gray-600">
            Cada tópico contém teoria, exemplos práticos e exercícios para fixação.
          </p>
        </div>
          ...
          {topics.map((topic) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">Texto {topic.name}</h3>
